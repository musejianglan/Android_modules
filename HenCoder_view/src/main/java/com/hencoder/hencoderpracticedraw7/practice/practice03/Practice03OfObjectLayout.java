...
public class Practice03OfObjectLayout extends RelativeLayout {
    ...
    private class PointFEvaluator implements TypeEvaluator<PointF> {
        ...
        public PointF evaluate(float fraction, PointF startValue, PointF endValue) {
            ...
            float x = (float) (startValue.x + (endValue.x - startValue.x) * Math.sin(fraction / 2 * Math.PI));
            float y = (float) (startValue.y + (endValue.y - startValue.y) * Math.sin(fraction/ 2 * Math.PI));
            mPointF.set(x,y);
