package com.musejianglan.customview.view1_2_paint_detail.practice;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.BitmapShader;
import android.graphics.Canvas;
import android.graphics.ComposeShader;
import android.graphics.Paint;
import android.graphics.PorterDuff;
import android.graphics.Shader;
import android.support.annotation.Nullable;
import android.util.AttributeSet;
import android.view.View;

import com.musejianglan.customview.R;

public class Practice05ComposeShaderView extends View {
    Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);

    public Practice05ComposeShaderView(Context context) {
        super(context);
    }

    public Practice05ComposeShaderView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public Practice05ComposeShaderView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    {
        setLayerType(LAYER_TYPE_SOFTWARE, null); // 硬件加速下 ComposeShader 不能使用两个同类型的 Shader

        //ComposeShader() 在硬件加速下是不支持两个相同类型的 Shader 的，所以这里也需要关闭硬件加速才能看到效果。


        // 用 Paint.setShader(shader) 设置一个 ComposeShader
        // Shader 1: BitmapShader 图片：R.drawable.batman
        // Shader 2: BitmapShader 图片：R.drawable.batman_logo

        Bitmap bitmap1 = BitmapFactory.decodeResource(getResources(), R.drawable.batman);
        Bitmap bitmap2 = BitmapFactory.decodeResource(getResources(), R.drawable.batman_logo);

        Shader shader1 = new BitmapShader(bitmap1, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
        Shader shader2 = new BitmapShader(bitmap2, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);

        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.ADD);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.SCREEN);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.CLEAR);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.DARKEN);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.LIGHTEN);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.MULTIPLY);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.OVERLAY);

        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.DST);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.DST_ATOP);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.DST_IN);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.DST_OUT);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.DST_OVER);

        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.SRC);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.SRC_ATOP);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.SRC_IN);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.SRC_OUT);
        //Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.SRC_OVER);
        Shader shader = new ComposeShader(shader1,shader2, PorterDuff.Mode.XOR);
        paint.setShader(shader);
    }

    @Override
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);

        canvas.drawCircle(200, 200, 200, paint);
    }
}
